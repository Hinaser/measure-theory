...
## 22.1 背景：平均を情報に合わせて作り直す

通常の期待値 $\mathbb{E}[X]$ は、情報をすべて平均して一つの数を返す。一方、条件付き期待値 $\mathbb{E}[X\mid\mathcal{G}]$ は、$\mathcal{G}$ で見える情報を残したまま平均する。

現実の予測では、何も知らない平均より、手元の情報に基づく予測が欲しい。患者の検査値を知った後のリスク、今日までの価格を知った後の明日の期待値、観測データを知った後の潜在変数の平均などがその例である。

:::tip[直感]
条件付き期待値は、「いま持っている情報で作れる最良の平均的予測」である。したがって数ではなく、情報に応じて値が変わる確率変数になる。
:::

Radon-Nikodym 定理による定義は抽象的に見えるが、役割は明確である。$\mathcal{G}$ で見えるどの事象の上でも、条件付き期待値の積分が元の $X$ の積分と一致するように作る。
