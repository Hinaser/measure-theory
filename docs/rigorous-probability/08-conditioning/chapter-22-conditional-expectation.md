...
## 22.10 読み物：情報が変わると、同じ量の平均も変わる

条件付き期待値を理解する小さな思考実験として、封筒の中の金額を考えよう。金額を $X$ とし、最初は何も見えないとする。この時点で使える最良の平均的予測は定数 $\mathbb{E}[X]$ である。次に封筒の色だけが見えたなら、予測は色ごとに変わる。この「色から判定できる事象の全体」が σ-代数 $\mathcal{G}$ であり、$\mathbb{E}[X\mid\mathcal{G}]$ は、知った情報の範囲内で更新された予測である。封筒を開ければ情報は最大になり、予測は $X$ 自身に戻る。

この考え方の源流には、賭けや保険で「現在までに分かったこと」を使って将来の損益を評価する問題がある。20世紀に確率論が測度論の言葉で整備されると、条件づけの対象は一つの事象 $B$ から、情報全体を表す σ-代数へ広がった。さらに Radon--Nikodym 定理によって、条件付き期待値は点ごとの割り算ではなく、すべての $A\in\mathcal{G}$ に対して積分が一致する量として構成できるようになった。これは $P(B)=0$ の場合に $P(A\mid B)$ が割り算では定義できない問題も避ける。

$L^2$ 射影という見方では、条件付き期待値は「情報から計算できる候補のうち、平均二乗誤差を最小にするもの」である。この連想は回帰、信号推定、フィルタリングへ直結する。ただし現実の観測装置や学習データが σ-代数そのものなのではない。σ-代数は、何を区別できるかを抽象化したモデルであり、センサーの丸め誤差や欠測をどの程度モデルへ入れるかは別途決めなければならない。

:::tip[式の読み方]
塔性質 $\mathbb{E}[\mathbb{E}[X\mid\mathcal{G}]] = \mathbb{E}[X]$ は、「情報を使って予測を細分化しても、それを情報を知らない立場でもう一度平均すれば元の平均に戻る」と読む。
:::
